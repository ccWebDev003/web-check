...
const Results = (): JSX.Element => {
	...
	const [lighthouseResults, updateLighthouseResults] = useMotherHook({
		...
		fetchRequest: () =>
			fetch(`${api}/quality?url=${address}`)
				.then(res => parseJson(res))
				.then(
					res => res?.lighthouseResult || { error: res.error || 'No Data' }
				),
	})
	...
	const [httpSecurityResults, updateHttpSecurityResults] = useMotherHook({
		...
		addressInfo: { address, addressType, expectedAddressTypes: urlTypeOnly },
		fetchRequest: () =>
			fetch(`${api}/http-security?url=${address}`).then(res => parseJson(res)),
	})
	...
	return (
		<ResultsOuter className='res-out'>
			...
			/>
			{address?.includes(window?.location?.hostname || 'riacoders.uz') && (
				<SelfScanMsg />
			)}
			<Loader
				show={
				...
				{showFilters ? (
					<>
						<div className='one-half'>
							<span className='group-label'>Filtrlash</span>
						...
						<div className='one-half'>
							<span className='group-label'>Qidirish</span>
							<input
								type='text'
					...
					<div className='control-options'>
						...
						<a href='#view-download-raw-data'>
							<span className='toggle-filters'>Ma'lumotlarni yuborish</span>
						</a>
						<a href='/about'>
							<span className='toggle-filters'>Natijani o'rganish</span>
						</a>
						<a href='/about#additional-resources'>
							<span className='toggle-filters'>Boshqa texnologiyalar</span>
						</a>
						<a href='https://github.com/lissy93/web-check'>
							<span className='toggle-filters'>GitHubdan ko'rish</span>
						</a>
					</div>
				)}
			</FilterButtons>
